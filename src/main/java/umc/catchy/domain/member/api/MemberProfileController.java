...
public class MemberProfileController {
    ...
    public ResponseEntity<BaseResponse<ProfileResponse>> getProfile() {
        return ResponseEntity.ok(BaseResponse.onSuccess(SuccessStatus._OK, memberProfileFacade.getCurrentMember()));
    }

    @PostMapping("/mypage/nickname")
    @Operation(summary = "닉네임 중복 검사 API", description = "변경하려는 닉네임이 중복인지 검사")
    public ResponseEntity<BaseResponse<Void>> validateNickname(@RequestBody @Valid NicknameRequest request) {
        memberProfileFacade.validateNickname(request);
        return ResponseEntity.ok(BaseResponse.onSuccess(SuccessStatus.NICKNAME_AVAILABLE, null));
    }

    @PatchMapping("/mypage/nickname")
    @Operation(summary = "닉네임 변경 API", description = "현재 로그인된 사용자의 닉네임 변경")
    public ResponseEntity<BaseResponse<NicknameResponse>> updateNickname(@RequestBody @Valid NicknameRequest request) {
        return ResponseEntity.ok(BaseResponse.onSuccess(SuccessStatus._OK, memberProfileFacade.updateNickname(request)));
    }
    ...
    public ResponseEntity<BaseResponse<ProfileImageResponse>> updateProfileImage(@RequestPart @Valid MultipartFile profileImage) {
        if (profileImage.isEmpty()) {
            throw new GeneralException(ErrorStatus.PROFILE_IMAGE_EMPTY);
        }
        return ResponseEntity.ok(BaseResponse.onSuccess(SuccessStatus._OK, memberProfileFacade.updateProfileImage(profileImage)));
    }
